...
class Point(object):
    ...
    def closer_to(self, m, n):
        dm = self.get_distance_from(m)
        dn = self.get_distance_from(n)
        if dm <= dn:
            return m
        else:
            return n
    ...
    def halfway_to(self, p):
        return "Point(" + str((p.x-self.x)/2 + self.x2) + ", " + str((p.y-self.y)/2 + self.y2) + ")"
